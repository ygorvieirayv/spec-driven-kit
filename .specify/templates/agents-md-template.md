...
O rigor é definido por feature. A spec registra os **limites de fidelidade** (real, simulado/sandbox e fora
de escopo); o plano seleciona os perfis de prova aplicáveis (`visual`, `logic`, `journey`, `data-security`,
`operational`, `delivery`); e `tasks.md` é a fonte canônica das tasks de toda feature formal. Achados
**Crítico** e **Alto** bloqueiam sempre até correção, nova verificação e novo review.
...
todos de uma vez:
- `constitution.md` — princípios, barra única de integridade e régua de cerimônia por risco da feature.
...
- Disjuntor anti-loop: se a mesma coisa falhar duas vezes seguidas sem progresso observável, pare, resuma o
  que tentou e devolva opções ao usuário.
